...
c	
c
c	Last modification: 28.03.2014 by Dr. Dongdong Zhang
c
	IMPLICIT none
c
	real*8  fnew
	character*128 filename1,filename2,arg
	character*8 fchar 
	...
	PARAMETER(IXRED=400,IYRED=100,IZRED=100)
c
...
c	------------------------------------------------------------
c	call subroutine readpa read potential from pa file
c	------------------------------------------------------------
...
c
c	------------------------------------------------------------
c	call subroutine rdc reduce the potential dimension
c	------------------------------------------------------------
...
c
c	------------------------------------------------------------
c	call subroutine write_output write voltage, stark energy, a
c								 acceleration , dimensions 
c	------------------------------------------------------------
...
c
c	XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
c	WRITEHEAD (not called in the program)
c	XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
c
...
c
c	XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
c	shuffle (not called in the program)
c	XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
c
...
c
c	XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
c	FFORMT (not called in the program)
c	XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
c
c	SUBROUTINE FFORMT(LINE,ISTART,LENGTH,VALUE)
c  	DECODES A FLOATING-POINT NUMBER OF GIVEN FIELD-POSITION & WIDTH
c	implicit real*8 (a-h,o-z)
c	CHARACTER          LINE(145),BUF(20)
c	CHARACTER*20 VBUF
c	CHARACTER*80 SPEC
c	EQUIVALENCE (BUF(1),VBUF)
c	IF (LENGTH.EQ.0) RETURN
c	DO 1 I=ISTART,ISTART+LENGTH-1
c1 		BUF(I-ISTART+1)=LINE(I)
c     	WIDTH=LENGTH
c      	WRITE (SPEC,10)       WIDTH
c10		FORMAT('(F',        F4.1,')')
c    	READ (VBUF,SPEC,ERR=2) VALUE
c   	RETURN
c2	WRITE (6,11) ISTART-1,(LINE(I),I=2,145)
c11	FORMAT (' I CAN''T READ FIELD STARTING AT CHARACTER',I4,'IN THE 1IS LINE:'/(1X,72A1))
c 	RETURN
c 	END
c
c	XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
c	reduce the array to the size for calculation
c	XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
c
...
c	XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
c	write_output to file for calculation
c	XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
c
...
c	close(ovunit)	
c
	open(unit=oeunit,file='../output/outEst.dat',
     .                 status='unknown')
	...
	close(oeunit)
c
	open(unit=oaxunit,file='../output/outax.dat',
     .                  status='unknown')
	...
	close(oaxunit)
c
	open(unit=oayunit,file='../output/outay.dat',
     .                  status='unknown')
	...
	close(oayunit)
c
	open(unit=oazunit,file='../output/outaz.dat',
     .                  status='unknown')
...
c
	open(unit=ocyzunit,file='../output/outcx.dat',
     .                   status='unknown')
	k=(nk+1)/2
	j=(nj+1)/2
	write(*,*) k
